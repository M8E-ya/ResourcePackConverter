...
public class SpawnEggConverter1_21_5 extends Converter {
    ...
    public void convert(Pack pack) throws IOException {
        ...
        if (baseSpawnEgg.toFile().exists()) {
            Logger.log("Found base spawn_egg.png, generating individual spawn eggs for 1.21.5+");
            for (String mob : MOBS) {
                Path targetPath = itemDir.resolve(mob + "_spawn_egg.png");
                if (!targetPath.toFile().exists()) {
                    Files.copy(baseSpawnEgg, targetPath);
                }
            }
        }
    }
